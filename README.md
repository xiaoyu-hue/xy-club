...
> ⚠️ **重要提示**：本项目由 AI 辅助开发。后台鉴权为**单密码**机制，密码以明文存于本地 JSON，**未经专业安全审计**，请部署后第一时间修改默认密码。
...
- **默认管理密码 `xy888888`** — 明文存于 `db.json`，**部署后请立即修改**
...
- **密码明文存储** — `settings.adminPassword` 未做哈希或加盐，`db.json` 泄露即等于密码泄露
- **单密码单管理员** — 没有多用户、角色或权限分级
- **无速率限制** — 登录接口未做防爆破限流，建议部署时用反向代理补上
- **无 CSRF 防护** — 接口依赖同源调用，跨域场景下需自行加固
- **单进程文件读写** — 多实例 / 多副本部署会导致写冲突，请只跑一个进程
- **上传仅校验大小** — 限 8MB，未做 MIME 白名单与内容扫描
...
- **存储用户敏感信息** — 明文密码与无审计日志的设计不足以承担实名、证件等数据
